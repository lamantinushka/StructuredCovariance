...
class LogisticRegressionLangevin(object):
    ...
    def __init__(self, inp_dim, max_rank = 5):
        ...
        self.num = torch.nn.Parameter(torch.zeros(inp_dim), requires_grad=False)
        self.denom = 0.
        self.T = 0
        self.V = torch.nn.Parameter(torch.zeros([inp_dim, inp_dim]), requires_grad=False)
        self.Sigma = torch.nn.Parameter(torch.eye(inp_dim), requires_grad=False)
        self.W = None
        self.frozen = False
    ...
    def nll_loss(self, x, y, require_grads = False, random = True, n_samples = 12):
        """ 
        ...
        if random:
            for i in range(n_samples):
                ...
                if require_grads:
                    ...
                    grads[i] = -torch.mul(x.t(), (y * torch.sigmoid(-z * y))).t() - self.mu 
    ...
    def sample(self):
        if self.frozen:
            ...
            try:
                y = sample_full(self.num/self.denom, self.Sigma)
            except:
                y = sample_full(self.num/self.denom, torch.nn.Parameter(torch.eye(self.mu.shape[0]), requires_grad=False))
        return y
